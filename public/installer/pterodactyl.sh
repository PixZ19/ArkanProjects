...
print_banner() {
    clear
    echo ""
    echo -e "${C_NEON_CYAN}${C_BOLD}  ArkanProjects${C_RESET}"
...
main_menu() {
    ...
    echo -e "  ${C_NEON_GREEN}4${C_RESET}  ${C_WHITE}Instal Blueprint, Theme & Addon${C_RESET}"
    echo ""

    local action=""
    while [ -z "$action" ]; do
        echo -e -n "  ${C_NEON_CYAN}>${C_RESET} Pilih [1-4]: "
        read -r action
        ...
        case "$action" in
            ...
            4) install_blueprint_menu; return ;;
            *) warn "Pilihan tidak valid. Masukkan 1, 2, 3, atau 4."; action="" ;;
        esac
    done
}
...
# ======================== INSTALASI BLUEPRINT/THEME/ADDON ========================

install_blueprint_menu() {
    while true; do
        print_banner
        echo -e "${C_NEON_PURPLE}${C_BOLD}  INSTALASI BLUEPRINT, THEME & ADDON${C_RESET}"
        echo ""
        echo -e "  ${C_NEON_GREEN}1${C_RESET}  ${C_WHITE}Blueprint${C_RESET} ${C_DIM}— file konfigurasi egg/server untuk Pterodactyl${C_RESET}"
        echo -e "  ${C_NEON_GREEN}2${C_RESET}  ${C_WHITE}Theme${C_RESET} ${C_DIM}— tema panel kustom dari komunitas${C_RESET}"
        echo -e "  ${C_NEON_GREEN}3${C_RESET}  ${C_WHITE}Addon${C_RESET} ${C_DIM}— ekstensi/modifikasi untuk panel${C_RESET}"
        echo -e "  ${C_NEON_GREEN}0${C_RESET}  ${C_WHITE}Kembali${C_RESET}"
        echo ""

        local choice=""
        echo -e -n "  ${C_NEON_CYAN}>${C_RESET} Pilih [0-3]: "
        read -r choice
        ...
        echo ""
        print_divider
        echo -e -n "  ${C_NEON_PURPLE}${C_BOLD}[?]${C_RESET} ${C_WHITE}Kembali ke menu utama?${C_RESET} (Y/n): "
        local back=""
        read -r back
        [[ "$back" =~ [Nn] ]] || return
    done
...
install_blueprints() {
    print_section "INSTALASI BLUEPRINTS"

    if [ ! -d "/var/www/pterodactyl" ]; then
        error_exit "Pterodactyl Panel belum terinstal. Instal Panel terlebih dahulu."
    fi

    info "Blueprint adalah file konfigurasi egg/server untuk Pterodactyl"
    info "Blueprint diunduh dari repositori komunitas"
    echo ""

    echo -e "  ${C_NEON_GREEN}1${C_RESET}  ${C_WHITE}Blueprint Minecraft (Vanilla, Fabric, Forge, Spigot)${C_RESET}"
    echo -e "  ${C_NEON_GREEN}2${C_RESET}  ${C_WHITE}Blueprint Server (CS2, Rust, FiveM, TF2)${C_RESET}"
    echo -e "  ${C_NEON_GREEN}3${C_RESET}  ${C_WHITE}Blueprint Database (PostgreSQL, MongoDB)${C_RESET}"
    echo -e "  ${C_NEON_GREEN}4${C_RESET}  ${C_WHITE}Blueprint Lainnya${C_RESET}"
    echo -e "  ${C_NEON_GREEN}0${C_RESET}  ${C_WHITE}Kembali${C_RESET}"
    echo ""

    local bp_choice=""
    echo -e -n "  ${C_NEON_CYAN}>${C_RESET} Pilih blueprint [0-4]: "
    read -r bp_choice

    case "$bp_choice" in
        0) return ;;
        1)
            step_header 1 3 "Mengunduh Blueprint Minecraft..."
            local bp_url="https://raw.githubusercontent.com/pterodactyl/panel/develop/app/Services/Eggs/Minecraft/minecraft.json"
            local bp_dir="/var/www/pterodactyl/blueprints"
            mkdir -p "$bp_dir"
            curl -sL "$bp_url" -o "$bp_dir/minecraft.json" 2>/dev/null
            if [ $? -ne 0 ]; then
                warn "Gagal mengunduh blueprint Minecraft dari repositori resmi."
                info "Mencoba repositori komunitas..."
                curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/minecraft/egg-vanilla-minecraft.json" -o "$bp_dir/minecraft-vanilla.json" 2>/dev/null \
                    && success "Blueprint Minecraft Vanilla terunduh dari komunitas" \
                    || error_exit "Gagal mengunduh blueprint Minecraft."
            else
                success "Blueprint Minecraft terunduh"
            fi
            ;;
        2)
            step_header 1 3 "Mengunduh Blueprint Server..."
            local bp_dir="/var/www/pterodactyl/blueprints"
            mkdir -p "$bp_dir"
            curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/game_eggs/csgo/csgo.json" -o "$bp_dir/cs2.json" 2>/dev/null \
                && success "Blueprint CS2 terunduh" \
                || warn "Gagal mengunduh blueprint CS2."
            curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/game_eggs/rust/rust.json" -o "$bp_dir/rust.json" 2>/dev/null \
                && success "Blueprint Rust terunduh" \
                || warn "Gagal mengunduh blueprint Rust."
            ;;
        3)
            step_header 1 3 "Mengunduh Blueprint Database..."
            local bp_dir="/var/www/pterodactyl/blueprints"
            mkdir -p "$bp_dir"
            curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/database/postgres.json" -o "$bp_dir/postgresql.json" 2>/dev/null \
                && success "Blueprint PostgreSQL terunduh" \
                || warn "Gagal mengunduh blueprint PostgreSQL."
            curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/database/mongodb.json" -o "$bp_dir/mongodb.json" 2>/dev/null \
                && success "Blueprint MongoDB terunduh" \
                || warn "Gagal mengunduh blueprint MongoDB."
            ;;
        4)
            step_header 1 3 "Mengunduh Blueprint Lainnya..."
            local bp_dir="/var/www/pterodactyl/blueprints"
            mkdir -p "$bp_dir"
            curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/game_eggs/source-engine/tf2.json" -o "$bp_dir/tf2.json" 2>/dev/null \
                && success "Blueprint TF2 terunduh" \
                || warn "Gagal mengunduh blueprint TF2."
            curl -sL "https://raw.githubusercontent.com/parkervcp/eggs/master/game_eggs/fivem/fivem.json" -o "$bp_dir/fivem.json" 2>/dev/null \
                && success "Blueprint FiveM terunduh" \
                || warn "Gagal mengunduh blueprint FiveM."
            ;;
        *)
    ...
    step_header 2 3 "Mengimpor blueprint ke Panel..."
    cd /var/www/pterodactyl || error_exit "Gagal masuk ke direktori Panel."

    # Set proper ownership
    case "$OS" in
        debian|ubuntu) chown -R www-data:www-data /var/www/pterodactyl/blueprints/ 2>/dev/null || true ;;
        rocky|almalinux) chown -R nginx:nginx /var/www/pterodactyl/blueprints/ 2>/dev/null || true ;;
    esac

    # Try importing via artisan if available
    if php artisan p:blueprint:install --help &>/dev/null; then
        for bp_file in /var/www/pterodactyl/blueprints/*.json; do
            [ -f "$bp_file" ] || continue
            info "Mengimpor $(basename "$bp_file")..."
            php artisan p:blueprint:install "$bp_file" --yes 2>/dev/null \
                && success "$(basename "$bp_file") berhasil diimpor" \
                || warn "Gagal mengimpor $(basename "$bp_file"). Impor manual melalui Admin Panel."
        done
    else
        info "Perintah blueprint:import tidak tersedia di versi Panel ini."
        info "Blueprint telah diunduh ke /var/www/pterodactyl/blueprints/"
        info "Impor blueprint secara manual melalui Admin Panel > Nests."
    fi

    step_header 3 3 "Membersihkan cache..."
    php artisan config:cache 2>/dev/null || true
    php artisan view:cache 2>/dev/null || true
    php artisan route:cache 2>/dev/null || true
    success "Cache dibersihkan"

    success "Instalasi blueprint selesai"
    log "=== INSTALASI BLUEPRINT SELESAI ==="
}

install_themes() {
    print_section "INSTALASI THEME"

    if [ ! -d "/var/www/pterodactyl" ]; then
        error_exit "Pterodactyl Panel belum terinstal. Instal Panel terlebih dahulu."
    fi

    echo -e "  ${C_NEON_GREEN}1${C_RESET}  ${C_WHITE}Pterodactyl Blue Admin Theme${C_RESET}"
    echo -e "  ${C_NEON_GREEN}2${C_RESET}  ${C_WHITE}Pterodactyl Twilight Theme${C_RESET}"
    echo -e "  ${C_NEON_GREEN}3${C_RESET}  ${C_WHITE}Pterodactyl Styx Theme${C_RESET}"
    echo -e "  ${C_NEON_GREEN}0${C_RESET}  ${C_WHITE}Kembali${C_RESET}"
    echo ""

    local theme_choice=""
    echo -e -n "  ${C_NEON_CYAN}>${C_RESET} Pilih theme [0-3]: "
    read -r theme_choice

    local theme_name=""
    local theme_repo=""
    local theme_branch="main"

    case "$theme_choice" in
        0) return ;;
        1)
            theme_name="pterodactyl-blue-admin"
            theme_repo="https://github.com/MuLTiAcidi/Pterodactyl-Blue-Admin-Theme.git"
            ;;
        2)
            theme_name="pterodactyl-twilight"
            theme_repo="https://github.com/Pterodactyl-Themes/Twilight.git"
            ;;
        3)
            theme_name="pterodactyl-styx"
            theme_repo="https://github.com/Pterodactyl-Themes/Styx.git"
            ;;
        *)
            warn "Pilihan tidak valid."
            return
    ...
    step_header 1 4 "Mengunduh theme ${theme_name}..."
    local tmp_dir="/tmp/${theme_name}"
    rm -rf "$tmp_dir"
    git clone --depth=1 "$theme_repo" "$tmp_dir" -b "$theme_branch" 2>/dev/null \
        || error_exit "Gagal mengklon repositori theme. Pastikan koneksi internet stabil."
    success "Theme terunduh ke ${tmp_dir}"

    step_header 2 4 "Memasang theme ke Panel..."
    cd /var/www/pterodactyl || error_exit "Gagal masuk ke direktori Panel."

    # Backup existing theme if any
    if [ -d "resources/views/themes" ]; then
        local backup_dir="resources/views/themes-backup-$(date +%Y%m%d%H%M%S)"
        cp -r resources/views/themes "$backup_dir" 2>/dev/null || true
        info "Backup theme lama disimpan di ${backup_dir}"
    fi

    # Copy theme files
    if [ -d "$tmp_dir/resources/views" ]; then
        cp -r "$tmp_dir/resources/views/"* resources/views/ 2>/dev/null || true
    fi
    if [ -d "$tmp_dir/public" ]; then
        cp -r "$tmp_dir/public/"* public/ 2>/dev/null || true
    fi
    # Fallback: copy everything from root if no standard structure
    if [ -f "$tmp_dir/index.blade.php" ] || [ -d "$tmp_dir/admin" ]; then
        mkdir -p resources/views/themes/"${theme_name}"
        cp -r "$tmp_dir/"* "resources/views/themes/${theme_name}/" 2>/dev/null || true
    fi
    success "File theme dipasang"

    step_header 3 4 "Mengatur permission..."
    case "$OS" in
        debian|ubuntu) chown -R www-data:www-data /var/www/pterodactyl/resources/ /var/www/pterodactyl/public/ 2>/dev/null || true ;;
        rocky|almalinux) chown -R nginx:nginx /var/www/pterodactyl/resources/ /var/www/pterodactyl/public/ 2>/dev/null || true ;;
    esac
    chmod -R 755 storage/* bootstrap/cache/ 2>/dev/null || true
    success "Permission diatur"

    step_header 4 4 "Membersihkan cache..."
    ...
    success "Theme ${theme_name} berhasil diinstal"
    info "Jika theme tidak berubah secara otomatis, atur theme aktif melalui Admin Panel."
    log "=== INSTALASI THEME SELESAI: ${theme_name} ==="
}

install_addons() {
    print_section "INSTALASI ADDONS"

    if [ ! -d "/var/www/pterodactyl" ]; then
        error_exit "Pterodactyl Panel belum terinstal. Instal Panel terlebih dahulu."
    fi

    echo -e "  ${C_NEON_GREEN}1${C_RESET}  ${C_WHITE}Billing Extension (Pterodactyl Billing)${C_RESET}"
    echo -e "  ${C_NEON_GREEN}2${C_RESET}  ${C_WHITE}Give Eggs via Discord${C_RESET}"
    echo -e "  ${C_NEON_GREEN}3${C_RESET}  ${C_WHITE}Advanced Ban System${C_RESET}"
    echo -e "  ${C_NEON_GREEN}0${C_RESET}  ${C_WHITE}Kembali${C_RESET}"
    echo ""

    local addon_choice=""
    echo -e -n "  ${C_NEON_CYAN}>${C_RESET} Pilih addon [0-3]: "
    read -r addon_choice

    local addon_name=""
    local addon_repo=""
    local addon_branch="main"

    case "$addon_choice" in
        0) return ;;
        1)
            addon_name="pterodactyl-billing"
            addon_repo="https://github.com/ArkanProjects/Pterodactyl-Billing.git"
            ;;
        2)
            addon_name="give-eggs-discord"
            addon_repo="https://github.com/MuLTiAcidi/pterodactyl-addons.git"
            addon_branch="main"
            ;;
        3)
            addon_name="advanced-ban-system"
            addon_repo="https://github.com/MuLTiAcidi/pterodactyl-addons.git"
            addon_branch="main"
            ;;
        *)
    ...
    step_header 1 5 "Mengunduh addon ${addon_name}..."
    local tmp_dir="/tmp/${addon_name}"
    rm -rf "$tmp_dir"
    git clone --depth=1 "$addon_repo" "$tmp_dir" -b "$addon_branch" 2>/dev/null \
        || error_exit "Gagal mengklon repositori addon. Pastikan koneksi internet stabil."
    success "Addon terunduh ke ${tmp_dir}"

    step_header 2 5 "Memasang addon ke Panel..."
    cd /var/www/pterodactyl || error_exit "Gagal masuk ke direktori Panel."

    # Copy addon files to appropriate locations
    ...
    if [ -d "$tmp_dir/resources/lang" ]; then
        cp -r "$tmp_dir/resources/lang/"* resources/lang/ 2>/dev/null || true
    fi
    if [ -d "$tmp_dir/public" ]; then
        cp -r "$tmp_dir/public/"* public/ 2>/dev/null || true
    fi
    if [ -d "$tmp_dir/database" ]; then
        cp -r "$tmp_dir/database/"* database/ 2>/dev/null || true
    fi
    if [ -d "$tmp_dir/routes" ]; then
        cp -r "$tmp_dir/routes/"* routes/ 2>/dev/null || true
    fi
    if [ -f "$tmp_dir/composer.json" ]; then
        info "Memeriksa dependensi Composer tambahan..."
    fi
    success "File addon dipasang"

    step_header 3 5 "Menginstal dependensi (jika ada)..."
    # Re-run composer to pick up new dependencies
    if [ -f "$tmp_dir/composer.json" ]; then
        COMPOSER_ALLOW_SUPERUSER=1 composer install --no-dev --optimize-autoloader --no-interaction 2>&1 | tail -3
    fi
    success "Dependensi diperbarui"

    step_header 4 5 "Menjalankan migrasi database..."
    php artisan migrate --force 2>/dev/null \
        && success "Migrasi database berhasil" \
        || info "Tidak ada migrasi baru atau migrasi sudah dijalankan."

    step_header 5 5 "Membersihkan cache..."
    php artisan view:clear 2>/dev/null || true
    php artisan config:clear 2>/dev/null || true
    php artisan cache:clear 2>/dev/null || true
    ...
    # Set permissions
    case "$OS" in
        debian|ubuntu) chown -R www-data:www-data /var/www/pterodactyl/ 2>/dev/null || true ;;
        rocky|almalinux) chown -R nginx:nginx /var/www/pterodactyl/ 2>/dev/null || true ;;
    esac
    ...
    success "Addon ${addon_name} berhasil diinstal"
    info "Restart worker queue jika diperlukan: systemctl restart pteroq"
    log "=== INSTALASI ADDON SELESAI: ${addon_name} ==="
}
...
main() {
    ...
    # Tanya apakah ingin menginstal Blueprint, Theme, atau Addon
    echo ""
    info "Apakah Anda ingin menginstal Blueprint, Theme, atau Addon?"
    echo -e -n "  ${C_NEON_CYAN}>${C_RESET} (y/N): "
    local install_bta=""
    read -r install_bta
    if [[ "$install_bta" =~ [Yy] ]]; then
        install_blueprint_menu
    fi
}
